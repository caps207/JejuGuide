...
<header class="main_menu">
		...
		<div class="container box_1170">
			<div class="row gallery-item">
				<div class="col-md-6">
					...
					<!-- 전체를 하나로 묶음 -->
					<h2 style="width: 1000px; text-align: left">명소 지도</h2>
 ...
 <script>
  ...
  var contentid = new Array(); //각각의 contentid 를 받아서 클릭시 상세정보로 보내주기
		...
		$.ajax({
					url : 'placeData.shop?pageNo=${pageNo}',
					async : false,
					type : 'get',
					dataType : 'json',
					success : function(data) {
						...
						for (var i = 0; i < myItem.length; i++) {
							...
							output += "<ul><div class='name'>"
									+ "<a href='placedetail.shop?contentid="
									+ myItem[i].contentid + "&contenttypeid="
									+ myItem[i].contenttypeid + "'><h4 class='mb-20'>" + myItem[i].title
									+ "</h4></a>"
							output += '<p>' + myItem[i].addr1 + '<p>'
							output += "<a href='placedetail.shop?contentid="
								+ myItem[i].contentid + "&contenttypeid="
								+ myItem[i].contenttypeid + "'><img src="+image+" style='width: 320px; height: 180px;'></a><br>"
							output += "</ul><br></div></div></div>" */
							output += "<div class='col-lg-4 col-md-4'>"
							output += "<a href='placedetail.shop?contentid=" + myItem[i].contentid + "&contenttypeid="+ myItem[i].contenttypeid+ "' class='place_btn'>"+"<h4>" +myItem[i].title + "</h4></a>"
							...
							output += "<a href='placedetail.shop?contentid=" + myItem[i].contentid + "&contenttypeid="+ myItem[i].contenttypeid+ "' class='place_btn'>상세보기</a>"
		...
		for (var i = 0; i < positions.length; i++) {
			...
			var iwContent = "<div><a href='placedetail.shop?contentid="
				...
				<c:if test="${totalCount > 0}">
					<c:if test="${pageNo > 1}">
						<li class="page-item active"><a
							href="place.shop?&pageNo=1"
							class="page-link"><<</a></li>
						<c:if test="${pageNo > 1}">
							<li class="page-item active"><a
								href="place.shop?pageNo=${pageNo - 1}"
								class="page-link"><</a></li>
						</c:if>
					</c:if>
					...
					<c:if test="${pageNo < maxpage}">
						<li class="page-item active"><a
							...
							class="page-link">></a></li>
						<li class="page-item active"><a
							href="place.shop?&pageNo=${maxpage}"
							class="page-link">>></a></li>
					</c:if>
				</c:if>
