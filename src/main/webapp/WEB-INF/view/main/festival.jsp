...
<!-- 	<div class="container">
		<div class="row justify-content-center">
			<div class="col-xl-6">
				...
				<div class="section_tittle text-center">
					<h2 style="width: 1000px; text-align: left">축제 지도 & 날씨</h2>
				</div>
			</div>
		</div>
	</div> -->
		...
		<div class="container box_1170">
			<div class="row gallery-item">
				<div class="col-md-6">
					...
					<!-- 전체를 하나로 묶음 -->
					<h2 style="width: 1000px; text-align: left">축제 지도</h2>
 ...
 <script>
 		...
 		/* $(function json()
		{
			...
			var myItem = data.response.body.items.item;
				for (var i = 0; myItem.length; i++) 
				{
					...
					"<h4 class='mb-20'>" + myItem[i].title + "</h4>"
					output += '<p>' + "주소:" +  myItem[i].addr1 + '<p>'
					output += '<p>' +  myItem[i].mapx + '<p>'
					output += "<img src="+image+" style='width: 320px; height: 180px;'><br>"
					output += "<a href='SearchDetail.shop?contentid="+myItem[i].contentid + "&contenttypeid="+myItem[i].contenttypeid + "'>세부정보</a>"
  			...
  			$.ajax({
					url : 'Festival.shop?pageNo=${pageNo}',
					async : false,
					type : 'get',
					dataType : 'json',
					success : function(data) {
						...
						//     output += "<div class='container'> <div class='row justify-content-center'><div class='col-xl-6'><br><br><br><hr><div class='section_tittle text-center'><h2 style='width: 1000px; text-align: left' >축제 정보</h2></div></div></div></div>";
						for (var i = 0; i < myItem.length; i++) 
						{
							...
							/* output += "<div class='col-md-4'><div class='single-defination'>"
							output += "<ul><div class='name'>"+ 
							"<a href='festivaldetail.shop?contentid="+myItem[i].contentid + "&contenttypeid="+myItem[i].contenttypeid + "'><h4 class='mb-20'>" + myItem[i].title + "</h4></a>"
							output += '<p>' + "주소:" +  myItem[i].addr1 + '<p>'
							output += "<a href='festivaldetail.shop?contentid="+myItem[i].contentid + "&contenttypeid="+myItem[i].contenttypeid + "'><img src="+image+" style='width: 320px; height: 180px;'></a><br>"
							output += "</ul><br></div></div></div>" */
							output += "<div class='col-lg-4 col-md-4'>"
							output += "<a href='festivaldetail.shop?contentid=" + myItem[i].contentid + "&contenttypeid="+ myItem[i].contenttypeid+ "' class='place_btn'>"+"<h4>" +myItem[i].title + "</h4></a>"
							...
							output += "<a href='festivaldetail.shop?contentid=" + myItem[i].contentid + "&contenttypeid="+ myItem[i].contenttypeid+ "' class='place_btn'>상세보기</a>"
							...
							title.push(myItem[i].title); //for문을 돌면서 각 title값을 title 리스트에 저장
							contentid.push(myItem[i].contentid); //for문을 돌면서 각 contentid값을 contentid 리스트에 저장
							//첫번째 질문 for문바깥으로 list를 출력하면 아무것도 안나옴.
							//두번째 질문 list(배열)값을 어떻게 hidden값으로 보낼 수 있는지.
		...
		for (var i = 0; i < positions.length; i++) {
			...
			//여기부분을 바꿔주면 infowindow 크기 변경 가능해~!
			var iwContent = "<div><a href='festivaldetail.shop?contentid="
					+ contentid[i] + "'>" + title[i] + "</div>", // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
			...
			kakao.maps.event.addListener(marker, 'click', makeListener(map,
					marker, infowindow));
		}
				...
				<c:if test="${totalCount > 0}">
					<c:if test="${pageNo > 1}">
						<li class="page-item active"><a
							href="festival.shop?&pageNo=1"
							class="page-link"><<</a></li>
						<c:if test="${pageNo > 1}">
							<li class="page-item active"><a
								href="festival.shop?pageNo=${pageNo - 1}"
								class="page-link"><</a></li>
						</c:if>
					</c:if>
					...
					<c:if test="${pageNo < maxpage}">
						<li class="page-item active"><a
							...
							class="page-link">></a></li>
						<li class="page-item active"><a
							href="festival.shop?&pageNo=${maxpage}"
							class="page-link">>></a></li>
					</c:if>
				</c:if>
