...
function win_login() {
	...
	$(function json()
	{
		...
		var output = '';
		console.log(myItem.length);
		output += "<div class='container'><h1><b>" + myItem.title + '</b></h1>'
		output += '<h4>' + myItem.addr1 + '</h4>'
		output += "<img src="+myItem.firstimage+" style='width:480px; height:360px'>"
		output += "<h4>" + myItem.overview + "</h4>"
		output += "<h4>" + myItem.homepage + "</h4>"
		output += "<br><br><br>";
		...
		output += "<input type='submit' value='스크랩'></form></div></div>";
		document.body.innerHTML += output;
	});
</script>
</head>
...
<!-- 	<div style="width: 40%; float: right; margin-right: 80px;">
		...
		<div style="width: 100%; height: 30%; float: right;">
			오른쪽 절반 하단

			<div id="show" style="float: left; width: 40%;"></div>

			<div style="border: 1px; float: right; width: 60%;">
				<img id="weatherImg" width="500px" height="400px" />
			</div>
			오른쪽 절반 하단 여기까지
	...
	<script>
		...
		$.ajax({
			url : 'SearchDetail.shop?contentid=' + contentid,
			async : false,
			type : 'get',
			dataType : 'json',
			success : function(data) {
